...
void FileMatrix(string,double **, int, int);
...
void MatrixInverse(double **A, int n)
{
  ...
  for(int j = 0; j < n; j++) {
    ...
    column[j] = 1.0;// column becomes ~f (RHS) in Project 1
    LUBackwardSubstitution(A, n, indx, column);// column overwritten to return x (the solution)
    // save result in y[][]
...
// Allocate memory for a matrix and initialize the elements to zero; m rows, n columns
...
void FileMatrix(string filename,double ** Matrix, int n,int m){
    FILE *ofile;
